...
public:
  ...
  using TaskHandle = uint32_t; /**< 软件定时任务句柄类型。 */
  ...
  static constexpr uint32_t kUseIntervalAsStartDelay = 0xFFFFFFFFUL; /**< 使用周期作为首次启动延时。 */
  ...
  struct TaskConfig final {
    ...
    uint8_t priority = kLowestPriority; /**< 调度优先级，数值越小优先级越高。 */
...
private:
  ...
  struct TaskSlot final {
    ...
    uint32_t requested_delay_ms = 0U; /**< 当前任务在回调内申请的延时。 */
    ...
    bool delay_requested = false; /**< 是否已申请延后执行。 */
